import { ErrorMapping, FailureCategory, Confidence } from "./types";
...
const MATCHER_SCORES: Record<"logPattern" | "errorMessage" | "errorCode", number> = {
  logPattern: 70,
  errorMessage: 40,
  errorCode: 20,
};

export function scoreToConfidence(score: number): "high" | "likely" | "uncertain" {
  ...
  for (const mapping of ERROR_MAPPINGS) {
    for (const priority of matcherPriority) {
      for (const matcher of mapping.matchers) {
        if (matcher.type !== priority) continue;
        ...
        if (matcher.type === "errorCode" && errorCode) {
          ...
          const pattern =
            matcher.pattern instanceof RegExp
              ? matcher.pattern
              : new RegExp(matcher.pattern as string, "i");
